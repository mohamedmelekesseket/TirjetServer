...
const uploadCategoryImage = async (req, res) => {
  try {
    ...
    console.error("UPLOAD CATEGORY IMAGE ERROR:", error.message);
...
const updateCategory = async (req, res) => {
  try {
    console.log("→ PUT body:", JSON.stringify(req.body, null, 2));  // ADD THIS
    ...
    console.log("→ Found category:", category.name);  // ADD THIS
    console.log("→ L2 count:", category.subcategories?.length);  // ADD THIS
    ...
    category.subcategories = (category.subcategories ?? []).map((l2) => {
    ...
    await category.save();
    console.log("→ Saved OK");  // ADD THIS

    res.status(200).json({ success: true, data: category, message: "Catégorie mise à jour" });
  } catch (error) {
    console.error("UPDATE CATEGORY ERROR FULL:", error);  // CHANGE THIS — log full error not just .message
    res.status(500).json({ success: false, message: error.message });
  }
};
...
const updateSubcategory = async (req, res) => {
  try {
    ...
    if (!l2)
      return res.status(404).json({ success: false, message: "L2 introuvable" });

    if (name  !== undefined) l2.name  = name;
    if (image !== undefined) l2.image = image;

    await category.save();
    res.status(200).json({ success: true, data: category, message: "L2 mis à jour" });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
const updateSubSubcategory = async (req, res) => {
  try {
    ...
    if (!l3)
      return res.status(404).json({ success: false, message: "L3 introuvable" });

    if (name  !== undefined) l3.name  = name;
    if (image !== undefined) l3.image = image;

    await category.save();
    res.status(200).json({ success: true, data: category, message: "L3 mis à jour" });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
const updateLevel4 = async (req, res) => {
  try {
    const { name, image } = req.body;
    const category = await Category.findById(req.params.id);
    if (!category)
    ...
    const l4 = l3.subcategories.id(req.params.itemId);
    if (!l4)
      return res.status(404).json({ success: false, message: "L4 introuvable" });

    if (name  !== undefined) l4.name  = name;
    if (image !== undefined) l4.image = image;

    await category.save();
    res.status(200).json({ success: true, data: category, message: "L4 mis à jour" });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
const seedCategories = async (req, res) => {
  try {
    const defaultCategories = [
      {
        name: "Bijoux et accessoires", slug: "bijoux-et-accessoires",
        ...
        subcategories: [
          { name: "Bijoux", slug: "bijoux", subcategories: [
            { name: "Bijoux modernes", slug: "bijoux-modernes", subcategories: [
            ...
            { name: "Bijoux traditionnels", slug: "bijoux-traditionnels", subcategories: [
          ...
          { name: "Sacs", slug: "sacs", subcategories: [
            { name: "Types de sacs", slug: "types-de-sacs", subcategories: [
          ...
          { name: "Chapeaux", slug: "chapeaux", subcategories: [
            { name: "Types de chapeaux", slug: "types-de-chapeaux", subcategories: [
              { name: "Mdhall", slug: "mdhall" },
              { name: "Chechia", slug: "chechia" },
              { name: "Casquettes", slug: "casquettes" },
            ]},
          ]},
          { name: "Autres accessoires", slug: "autres-accessoires", subcategories: [
            { name: "Divers", slug: "divers-accessoires", subcategories: [
      ...
      {
        name: "Trésors de la gastronomie", slug: "tresors-de-la-gastronomie",
        mainCategory: "artisanat",
        image: "https://images.unsplash.com/photo-1558642452-9d2a7deb7f62?w=800&q=85",
        description: "Produits alimentaires artisanaux",
        subcategories: [
          { name: "Bsissa", slug: "bsissa", subcategories: [
            { name: "Types de bsissa", slug: "types-de-bsissa", subcategories: [
              { name: "Blé", slug: "bsissa-ble" }, { name: "Pois chiches", slug: "bsissa-pois-chiches" },
              { name: "Lentille", slug: "bsissa-lentille" }, { name: "Sorgho", slug: "bsissa-sorgho" },
              { name: "Fruits secs", slug: "bsissa-fruits-secs" }, { name: "Orge", slug: "bsissa-orge" },
              { name: "Sans gluten", slug: "bsissa-sans-gluten" }, { name: "En poudre", slug: "bsissa-en-poudre" },
              { name: "Mélangée", slug: "bsissa-melangee" }, { name: "Autres bsissa", slug: "bsissa-autres" },
            ]},
          ]},
          { name: "Confitures", slug: "confitures", subcategories: [
            { name: "Types de confitures", slug: "types-de-confitures", subcategories: [
          ...
          { name: "Graines et céréales", slug: "graines-et-cereales", subcategories: [
            { name: "Types de céréales", slug: "types-de-cereales", subcategories: [
              { name: "Couscous blé", slug: "couscous-ble" }, { name: "Couscous melthout", slug: "couscous-melthout" },
              { name: "Couscous", slug: "couscous" }, { name: "Produit sans gluten", slug: "produit-sans-gluten" },
              { name: "Borghol", slug: "borghol" }, { name: "Granola", slug: "granola" },
              { name: "Dechicha", slug: "dechicha" }, { name: "Rechta Eskef", slug: "rechta-eskef" },
              { name: "Chroba", slug: "chroba" }, { name: "Mhames", slug: "mhames" },
              { name: "Sorgho", slug: "sorgho" },
            ]},
          ]},
          { name: "Farines", slug: "farines", subcategories: [
            { name: "Types de farines", slug: "types-de-farines", subcategories: [
              { name: "Farine de blé", slug: "farine-ble" }, { name: "Farine d'orge", slug: "farine-orge" },
              { name: "Farine de maïs", slug: "farine-mais" }, { name: "Farine de flocons d'avoine", slug: "farine-avoine" },
            ]},
          ]},
          { name: "Miels", slug: "miels", subcategories: [
            { name: "Types de miels", slug: "types-de-miels", subcategories: [
              { name: "Miel de cèdre", slug: "miel-cedre" }, { name: "Miel de montagne", slug: "miel-montagne" },
              { name: "Miel de thym", slug: "miel-thym" }, { name: "Miel de romarin", slug: "miel-romarin" },
              { name: "Miel multiflore", slug: "miel-multiflore" }, { name: "Autres miels", slug: "miels-autres" },
            ]},
          ]},
          { name: "Huile d'olive", slug: "huile-dolive", subcategories: [
            { name: "Par région", slug: "huile-par-region", subcategories: [
              { name: "Huile de Gafsa", slug: "huile-gafsa" }, { name: "Huile de Douiret", slug: "huile-douiret" },
              { name: "Huile de Djerba", slug: "huile-djerba" }, { name: "Huile de Matmata", slug: "huile-matmata" },
              { name: "Huile de Sahel", slug: "huile-sahel" }, { name: "Huile de Sfax", slug: "huile-sfax" },
              { name: "Huile du nord", slug: "huile-nord" },
            ]},
            { name: "Olives", slug: "olives", subcategories: [
          ...
          { name: "Figues", slug: "figues", subcategories: [
            { name: "Types de figues", slug: "types-de-figues", subcategories: [
              ...
              { name: "Chriha", slug: "chriha" }, { name: "Gherbouz", slug: "gherbouz" },
            ]},
          ]},
          { name: "Viandes artisanales", slug: "viandes-artisanales", subcategories: [
            { name: "Types de viandes", slug: "types-de-viandes", subcategories: [
              { name: "Kaddid", slug: "kaddid" }, { name: "Ousben kerkouch", slug: "ousben-kerkouch" },
              { name: "Merguez", slug: "merguez" },
            ]},
          ]},
          { name: "Dattes", slug: "dattes", subcategories: [] },
          { name: "Épices et mélanges traditionnels", slug: "epices-melanges-traditionnels", subcategories: [
            { name: "Types d'épices", slug: "types-depices", subcategories: [
              { name: "Ail", slug: "ail" }, { name: "Yazoul", slug: "yazoul" },
              { name: "Harissa", slug: "harissa" }, { name: "Tomate séchée", slug: "tomate-sechee" },
            ]},
          ]},
          { name: "Fromages", slug: "fromages", subcategories: [] },
          { name: "Pâtisseries", slug: "patisseries", subcategories: [
            { name: "Types de pâtisseries", slug: "types-de-patisseries", subcategories: [
          ...
          { name: "Herbes médicinales", slug: "herbes-medicinales", subcategories: [
            { name: "Types d'herbes", slug: "types-dherbes", subcategories: [
              { name: "Romarin", slug: "romarin" }, { name: "Armoise", slug: "armoise" },
            ]},
          ]},
        ],
      },
      {
        name: "Produits du terroir", slug: "produits-du-terroir",
        mainCategory: "artisanat",
        image: "https://images.unsplash.com/photo-1464226184884-fa280b87c399?w=800&q=85",
        description: "Produits locaux authentiques",
        subcategories: [
          { name: "Produits laitiers", slug: "produits-laitiers", subcategories: [
            { name: "Types de produits laitiers", slug: "types-produits-laitiers", subcategories: [
          ...
          { name: "Produits régionaux", slug: "produits-regionaux", subcategories: [
            { name: "Types de produits régionaux", slug: "types-produits-regionaux", subcategories: [
      ...
      {
        name: "Coffrets cadeaux", slug: "coffrets-cadeaux",
        mainCategory: "artisanat",
        image: "https://images.unsplash.com/photo-1549465220-1a8b9238cd48?w=800&q=85",
        description: "Coffrets artisanaux et personnalisés",
        subcategories: [
          { name: "Types de coffrets", slug: "types-de-coffrets", subcategories: [
            { name: "Coffrets thématiques", slug: "coffrets-thematiques", subcategories: [
      ...
      {
        name: "Vêtements et chaussures", slug: "vetements-et-chaussures",
        mainCategory: "artisanat",
        ...
        subcategories: [
          { name: "Vêtements", slug: "vetements", subcategories: [
            { name: "Vêtements traditionnels", slug: "vetements-traditionnels", subcategories: [
              ...
              { name: "Écharpe", slug: "echarpe" }, { name: "Bekhnoug", slug: "bekhnoug" },
              { name: "Kachabia", slug: "kachabia" }, { name: "Wezra", slug: "wezra" },
              { name: "Houli", slug: "houli" }, { name: "Jebba khemri", slug: "jebba-khemri" },
              { name: "Jebba sekrouda", slug: "jebba-sekrouda" }, { name: "Jebba homme", slug: "jebba-homme" },
              { name: "Jebba femme", slug: "jebba-femme" }, { name: "Koftane", slug: "koftane" },
              { name: "Taajira", slug: "taajira" }, { name: "Fouta et blouza", slug: "fouta-blouza" },
              { name: "Chemise chebka", slug: "chemise-chebka" },
              { name: "Chemise adass et kontil", slug: "chemise-adass-kontil" },
              { name: "Pul fadhila", slug: "pul-fadhila" }, { name: "Pul rouhia", slug: "pul-rouhia" },
            ]},
            { name: "Vêtements modernes inspirés amazigh", slug: "vetements-modernes-inspires-amazigh", subcategories: [
              { name: "Aassab traditionnel", slug: "aassab-traditionnel" },
              { name: "Aassab moderne", slug: "aassab-moderne" },
            ]},
          ]},
          { name: "Chaussures", slug: "chaussures", subcategories: [
            { name: "Types de chaussures", slug: "types-de-chaussures", subcategories: [
              { name: "Balgha", slug: "balgha" }, { name: "Sandales en cuir", slug: "sandales-cuir" },
      ...
      {
        name: "Maison et décoration", slug: "maison-et-decoration",
        mainCategory: "artisanat",
        ...
        subcategories: [
          { name: "Tissages amazigh", slug: "tissages-amazigh", subcategories: [
            { name: "Klim et margoum", slug: "klim-et-margoum", subcategories: [
            ...
            { name: "Coussins et couvertures", slug: "coussins-et-couvertures", subcategories: [
          ...
          { name: "Broderie amazighe", slug: "broderie-amazighe", subcategories: [
            { name: "Types de broderie", slug: "types-de-broderie", subcategories: [
              ...
              { name: "Parure", slug: "parure" }, { name: "Draps", slug: "draps" },
            ]},
          ]},
          { name: "Poterie et bois", slug: "poterie-et-bois", subcategories: [
          ...
          { name: "Tapis", slug: "tapis", subcategories: [
            { name: "Types de tapis", slug: "types-de-tapis", subcategories: [
              { name: "Tapis de haute laine", slug: "tapis-haute-laine" },
            ]},
          ]},
          { name: "Décoration murale", slug: "decoration-murale", subcategories: [
            { name: "Éléments décoratifs", slug: "elements-decoratifs", subcategories: [
              { name: "Articles en base de palmier", slug: "articles-palmier" },
              { name: "Miroir", slug: "miroir" }, { name: "Vases", slug: "vases" },
              { name: "Rideaux", slug: "rideaux" }, { name: "Pot de plante", slug: "pot-de-plante" },
            ]},
          ]},
          { name: "Luminaires et bougies", slug: "luminaires-et-bougies", subcategories: [
            { name: "Types de luminaires", slug: "types-de-luminaires", subcategories: [
              { name: "Lampes artisanales", slug: "lampes-artisanales" },
              { name: "Luminaires", slug: "luminaires" }, { name: "Porte-bougie", slug: "porte-bougie" },
              { name: "Bougies parfumées", slug: "bougies-parfumees" },
              { name: "Paniers", slug: "paniers" }, { name: "Couffin", slug: "couffin-deco" },
            ]},
          ]},
          { name: "Cuisine et arts de la table", slug: "cuisine-et-arts-de-la-table", subcategories: [
            { name: "Ustensiles et vaisselle", slug: "ustensiles-et-vaisselle", subcategories: [
      ...
      },
      {
        ...
        subcategories: [
          { name: "Soins du corps", slug: "soins-du-corps", subcategories: [
            { name: "Types de soins", slug: "types-de-soins", subcategories: [
              { name: "Savon traditionnel", slug: "savon-traditionnel" },
              { name: "Produits de hammam", slug: "produits-hammam" },
              { name: "Henna", slug: "henna" }, { name: "Herkous", slug: "herkous" },
              { name: "Produits de beauté bio", slug: "produits-beaute-bio" },
            ]},
          ]},
          { name: "Huiles et parfums", slug: "huiles-et-parfums", subcategories: [
            { name: "Types d'huiles", slug: "types-dhuiles", subcategories: [
          ...
          { name: "Soins pour cheveux", slug: "soins-cheveux", subcategories: [] },
        ],
      },
      {
        name: "Art et culture", slug: "art-et-culture",
        ...
        subcategories: [
          { name: "Peinture", slug: "peinture", subcategories: [
            { name: "Styles de peinture", slug: "styles-de-peinture", subcategories: [
          ...
          { name: "Objets culturels", slug: "objets-culturels", subcategories: [
            { name: "Patrimoine amazigh", slug: "patrimoine-amazigh", subcategories: [
      ...
      {
        name: "Événements et traditions", slug: "evenements-et-traditions",
        ...
        subcategories: [
          { name: "Mariage amazigh", slug: "mariage-amazigh-evenements", subcategories: [
            { name: "Produits et accessoires", slug: "produits-et-accessoires-mariage", subcategories: [
      ...
      {
        name: "Tourisme et loisir", slug: "tourisme-et-loisir",
        ...
        subcategories: [
          { name: "Hébergements", slug: "hebergements", subcategories: [
            { name: "Types d'hébergements", slug: "types-dhebergements", subcategories: [
      ...
      {
        name: "Langue amazigh", slug: "langue-amazigh",
    ...
    res.status(201).json({ success: true, message: `${created.length} catégories créées avec succès`, data: created });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
